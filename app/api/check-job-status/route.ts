...
export async function GET(request: NextRequest) {
  console.log('API route: Starting request processing for check-job-status');
  ...
  try {
    ...
    if (isDevelopment) {
      ...
      functionUrl = `${netlifyDevUrl}/.netlify/functions/check-job-status?jobId=${jobId}`;
      ...
      functionUrl = `${netlifyUrl}/.netlify/functions/check-job-status?jobId=${jobId}`;
